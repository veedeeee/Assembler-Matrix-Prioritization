# MOD Publishing Checklist

Checklist for publishing Assembler Matrix Prioritization to CurseForge and Modrinth.

## Pre-publish (Both Platforms)

- [x] Build artifacts are ready (generated by GitHub Actions on release)
  - [x] `neoforge/build/libs/assembler-matrix-prioritization-neoforge-*.jar`
  - [ ] `neoforge2612/build/libs/assembler-matrix-prioritization-neoforge-26.1.2-*.jar`
  - [x] `forge/build/libs/assembler-matrix-prioritization-forge-*.jar`
- [x] All release tests pass
  - [x] B-NF-211 PASS
  - [ ] B-NF-262 PASS
  - [x] B-FG-201 PASS
- [x] MOD icon ready — `images/mod_logo.png`
- [ ] Changelog written

---

## CurseForge

### Account & Project Setup (first time only)

- [x] Create/log in to CurseForge account at <https://authors.curseforge.com/>
- [x] Create new project — done
  - [x] Game: Minecraft
  - [x] Class: Mods
  - [x] Name: `Assembler Matrix Prioritization`
  - [x] Summary: (short description)
  - [x] Description: (English, sufficiently detailed)
  - [x] License: select appropriate license
  - [x] Main category: select appropriate category
  - [x] Logo image: uploaded — `images/mod_logo.png`
- [x] Note down the **Project ID** — CurseForge: `1631512`

### File Upload
> Note: File upload is automated by mc-publish GitHub Action on push to master.

- [ ] Navigate to project → Files tab → Upload File
- [ ] Upload NeoForge jar
  - [ ] File: `assembler-matrix-prioritization-neoforge-*.jar`
  - [ ] Display Name: e.g. `Assembler Matrix Prioritization vX.Y.Z (NeoForge 1.21.1)`
  - [ ] Release Type: `Release` / `Beta` / `Alpha`
  - [ ] Supported Versions: `1.21.1` + `NeoForge`
  - [ ] Changelog: paste changelog
  - [ ] Required Dependencies: `Applied Energistics 2`, `ExtendedAE`
- [ ] Upload NeoForge 26.1.2 jar
  - [ ] File: `assembler-matrix-prioritization-neoforge-26.1.2-*.jar`
  - [ ] Display Name: e.g. `Assembler Matrix Prioritization vX.Y.Z (NeoForge 26.1.2)`
  - [ ] Release Type: `Release` / `Beta` / `Alpha`
  - [ ] Supported Versions: `26.1.2` + `NeoForge`
  - [ ] Changelog: paste changelog
  - [ ] Required Dependencies: `Applied Energistics 2`, `ExtendedAE (26.1.2 alpha line)`
- [ ] Upload Forge jar
  - [ ] File: `assembler-matrix-prioritization-forge-*.jar`
  - [ ] Display Name: e.g. `Assembler Matrix Prioritization vX.Y.Z (Forge 1.20.1)`
  - [ ] Release Type: `Release` / `Beta` / `Alpha`
  - [ ] Supported Versions: `1.20.1` + `Forge`
  - [ ] Changelog: paste changelog
  - [ ] Required Dependencies: `Applied Energistics 2`, `ExtendedAE`
- [ ] Confirm status changes to `Under Review`

### Post-approval

- [ ] Verify public URL: `https://www.curseforge.com/minecraft/mc-mods/<slug>`
- [x] Automation set up via mc-publish GitHub Action

---

## Modrinth

### Account & Project Setup (first time only)

- [x] Create/log in to Modrinth account at <https://modrinth.com/>
- [x] Create new project — done
  - [x] Name: `Assembler Matrix Prioritization`
  - [x] Summary: (short description)
  - [x] Description: (detailed)
  - [x] License: select appropriate license
  - [x] Categories: select appropriate categories
  - [x] Icon: upload 256×256px icon
- [x] Project ID: `QELalDXk` (GitHub var: MODRINTH_PROJECT_ID)

### Version Upload
> Note: Version upload is automated by mc-publish GitHub Action on push to master.

- [ ] Navigate to project → Versions → Create version
- [ ] Upload NeoForge jar
  - [ ] File: `assembler-matrix-prioritization-neoforge-*.jar`
  - [ ] Version number: `vX.Y.Z`
  - [ ] Release channel: `Release` / `Beta` / `Alpha`
  - [ ] Game versions: `1.21.1`
  - [ ] Loaders: `NeoForge`
  - [ ] Changelog: paste changelog
  - [ ] Dependencies (required): `Applied Energistics 2`, `ExtendedAE`
- [ ] Upload NeoForge 26.1.2 jar
  - [ ] File: `assembler-matrix-prioritization-neoforge-26.1.2-*.jar`
  - [ ] Version number: `vX.Y.Z`
  - [ ] Release channel: `Release` / `Beta` / `Alpha`
  - [ ] Game versions: `26.1.2`
  - [ ] Loaders: `NeoForge`
  - [ ] Changelog: paste changelog
  - [ ] Dependencies (required): `Applied Energistics 2`, `ExtendedAE (26.1.2 alpha line)`
- [ ] Upload Forge jar
  - [ ] File: `assembler-matrix-prioritization-forge-*.jar`
  - [ ] Version number: `vX.Y.Z`
  - [ ] Release channel: `Release` / `Beta` / `Alpha`
  - [ ] Game versions: `1.20.1`
  - [ ] Loaders: `Forge`
  - [ ] Changelog: paste changelog
  - [ ] Dependencies (required): `Applied Energistics 2`, `ExtendedAE`
- [ ] Confirm status changes to `processing` → `approved`

### Post-approval

- [ ] Verify public URL: `https://modrinth.com/mod/<slug>`
- [x] Automation set up via mc-publish GitHub Action

---

## Automation Option (Both Platforms at Once)

- [x] Set up `mc-publish` GitHub Action — configured in `.github/workflows/release.yml`
  - Repository: <https://github.com/Kir-Antipov/mc-publish>
  - Requires: CurseForge API token, Modrinth PAT, both project IDs

---

## Notes

- Review time SLA is not officially stated for either platform; allow several days for first submission.
- First release requires project setup + first file upload. Subsequent releases only need file upload.
- Projects must have at least one `Release`-type file to sync to the CurseForge App.
